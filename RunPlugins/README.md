...
vim.v1.ahk is the setting file for [AutoHotkey](http://www.autohotkey.com/)(Autohotkey_L).

vim.exe is a standalone application made from vim.v1.ahk (available in [the releases page](https://github.com/rcmdnk/vim_ahk/releases)).
...
If you've already installed AutoHotKey, just open vim.v1.ahk with AutoHotkey.

If you are running AutoHotKey with another script,
you can include it in your script using AutoHotKey...
Please copy vim.v1.ahk and lib directory in `\Users\%username%\Documents`
or where the script is in,
and add the following line in AutoHotkey.v1.ahk or your script:

    #Include  %A_LineFile%\..\vim.v1.ahk
...
variable before `Vim := new VimAhk()` in vim.v1.ahk
(Window title/class can be checked by Window spy of AutoHotkey),
or write before including vim.v1.ahk
...
You can add your options before including **vim.v1.ahk** in your script
in the auto execute section like:

    VimVerbose := 2
    #Include \path\to\vim.v1.ahk

If you want to change them directly in the vim.v1.ahk script,
...
Tests are run by executing `tests/run_vimahk_tests.v1.ahk`. A notepad and vim window is opened, and vim_ahk is started.
